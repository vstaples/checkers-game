document.addEventListener('DOMContentLoaded', () => {
    ...
    function startDrag(event) {
        ...
        origLeft = 0; //rect.left - board.getBoundingClientRect().left;
        origTop = 0; //rect.top - board.getBoundingClientRect().top;
    ...
    function dropPiece(event) {
        ...
        if (targetSquare) {
            ...
            if (Math.abs(targetRow - draggedRow) === 1 && Math.abs(targetCol - draggedCol) === 1) {
                const isCrowned = draggedElement.classList.contains('crowned');
                if (!isCrowned) {
                    if (draggedElement.classList.contains('red') && targetRow < draggedRow) return;
                    if (draggedElement.classList.contains('black') && targetRow > draggedRow) return;
                }
